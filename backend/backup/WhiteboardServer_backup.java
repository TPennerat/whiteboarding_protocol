...
import fi.whiteboardaalto.messages.SuperMessage2;
        ...
        try {
            ...
            switch (msg.getType()) {
                ...
                case JOIN_MEETING:
                    ...
                    break;
                case CREATE_OBJECT:
                    System.out.println("CreateObject request received!!!");
                    SuperMessage2 test = mapper.readValue(message, SuperMessage2.class);
                    System.out.println("Printing object created...");
                    System.out.println(test.toString());

                    /*
                    CreateObject createObject = mapper.readValue(messageString, CreateObject.class);
                    String objectString = createObject.getWhiteBoardObject().toString();

                    JSONObject whiteboardObject = new JSONObject(objectString);
                    for (String key: whiteboardObject.keySet()) {
                        System.out.println("Key is: " + key +", value is: " + whiteboardObject.get(key));
                    }
                    */
                    ...
                    switch(createObject.getType()) {
                        case STICKY_NOTE:
                            StickyNote stickyNote = mapper.readValue(objectString, StickyNote.class);
                            break;
                    }
                    */
                    break;
